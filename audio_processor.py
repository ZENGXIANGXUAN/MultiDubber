...
def _trim_or_pad_to_target(output_path: str, y: np.ndarray, sr: int, target_duration_s: float):
    """Ensure audio is exactly target_duration_s by trimming or padding."""
    ...
    if current_samples > target_samples:
        ...
        # Pad with silence
        pad_count = target_samples - current_samples
        pad_shape = (pad_count, y.shape[1]) if y.ndim > 1 else (pad_count,)
        y = np.concatenate((y, np.zeros(pad_shape, dtype=y.dtype)))
...
def adjust_duration_with_rubberband(input_path: str, output_path: str, target_duration_s: float):
    try:
        ...
        if abs(rate - 1.0) < 0.001:
            _trim_or_pad_to_target(output_path, y, sr, target_duration_s)
            return
        if not USE_TQDM_PROGRESS_BAR:
        ...
        if rate < MIN_RUBBERBAND_RATE:
            ...
            _trim_or_pad_to_target(output_path, final_y, sr, target_duration_s)
        else:
            stretched_y = rb.time_stretch(y, sr, rate)
            _trim_or_pad_to_target(output_path, stretched_y, sr, target_duration_s)
    except Exception as e:
...
def merge_audio(parsed_subtitles: List[List], current_tmp_dir: str) -> AudioSegment:
    ...
    if first_start_sec > 0:
        merged_audio += AudioSegment.silent(duration=int(round(first_start_sec * 1000)))
    for i, subtitle in enumerate(parsed_subtitles):
        start_time_str, end_time_str, _, _ = subtitle
        target_duration_ms = int(round((time_str_to_seconds(end_time_str) - time_str_to_seconds(start_time_str)) * 1000))
        ...
        try:
            ...
            # If actual duration deviates from target by >50ms, log a warning
            if abs(actual_ms - target_duration_ms) > 50:
                _log("WARN", f"Task {i} 时长偏差: 实际 {actual_ms}ms vs 目标 {target_duration_ms}ms")
        ...
        if i < len(parsed_subtitles) - 1:
            ...
            actual_merged_duration_s = len(merged_audio) / 1000.0
            gap_ms = int(round((next_start_sec - actual_merged_duration_s) * 1000))
            if gap_ms > 0:
                merged_audio += AudioSegment.silent(duration=gap_ms)
    return merged_audio